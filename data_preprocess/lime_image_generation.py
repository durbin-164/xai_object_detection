...
output_path = '/home/masud.rana/Documents/Learning_Project/Important/XAI/data/output/lime'
...
def get_images_with_label(explanation):
    image_with_label = []
    for label in explanation.top_labels:
        ...
        image_with_label.append((img_boundry, selected_classes[label]))

    return image_with_label


def draw_images(images_with_label, save_path):
    fig, axes = plt.subplots(1, len(images_with_label), figsize=(15, 3))

    for i, img_with_label in enumerate(images_with_label):
        img, label = img_with_label
        axes[i].imshow(img)
        axes[i].axis('off')  # Turn off axis labels
        # Set the label at the top of each column
...
def generate_lime_values(image_path, save_path, true_labels):
    image = cv2.imread(image_path)

    # converting the image to RGB format. Opencv reads image in BGR format, so we need to convert
    ...
    explanation = explainer.explain_instance(image,
                                             predict_torch_batch,  # classification function
                                             top_labels=5,
                                             hide_color=0,
                                             num_samples=1000)  # number of images that will be sent to classification function

    images_with_label = [(image, true_labels)]
    images_with_label += get_images_with_label(explanation)

    draw_images(images_with_label, save_path)
...
def main():
    ...
    for class_name in os.listdir(input_path):
        ...
        for image_file in os.listdir(class_path):
            ...
            generate_lime_values(image_path, save_file, true_labels)
